...
- 最后一步必须回测向前 10 天已有推荐结果，并在 `runs/ashare-trend-buy/backtests/` 生成 `*_backtest_report.md` 和同名 CSV。回测口径必须包含推荐次日、5 个后续交易日、至今涨跌幅；所有统计和胜率必须基于推荐次日及之后的实际市场行情，不统计推荐当日涨跌幅。若历史报告不足 10 天，就按实际可用日期回测并说明样本数。
